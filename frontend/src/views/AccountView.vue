<template>
  <div>
    ...
    <ul class="nav nav-tabs mb-4">
      <li class="nav-item">
        <a class="nav-link" :class="{ active: activeTab === 'profil' }" href="#" @click.prevent="activeTab = 'profil'">
          <svg class="icon me-1" width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <li class="nav-item">
        <a class="nav-link" :class="{ active: activeTab === 'securite' }" href="#" @click.prevent="switchToSecurity">
          <svg class="icon me-1" width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
          ...
          <div class="d-flex gap-2 align-items-center">
            <span v-if="revokeSuccess" class="text-success small">
              <svg class="icon me-1" width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
              </svg>
              Sessions révoquées
            </span>
            <button class="btn btn-sm btn-outline-danger" @click="revokeAllSessions" :disabled="revokeLoading">
              <span v-if="revokeLoading" class="spinner-border spinner-border-sm me-1"></span>
              <svg v-else class="icon me-1" width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="table-responsive">
          <table class="table table-vcenter card-table">
            ...
            <tbody>
              <tr v-if="loginLoading">
                <td colspan="5" class="text-center text-secondary py-3">Chargement...</td>
              </tr>
              <tr v-else-if="!loginEvents.length">
                <td colspan="5" class="text-center text-secondary py-3">Aucune connexion enregistrée</td>
              </tr>
              <tr v-for="ev in loginEvents" :key="ev.id">
                ...
                <td>
                  <span class="d-flex align-items-center gap-1">
                    {{ ev.ip_address }}
                    <svg v-if="suspiciousIPs.has(ev.ip_address) && !ev.success" class="icon text-warning" width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24" title="Plusieurs échecs depuis cette IP">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/>
                    </svg>
                  </span>
                </td>
...
// IPs with 3+ failures in the login history
const suspiciousIPs = computed(() => {
  const counts = {}
  for (const ev of loginEvents.value) {
    if (!ev.success) {
      counts[ev.ip_address] = (counts[ev.ip_address] || 0) + 1
    }
  }
  return new Set(Object.entries(counts).filter(([, c]) => c >= 3).map(([ip]) => ip))
})
...
async function loadLoginEvents() {
  ...
  try {
    const { data } = await apiClient.getLoginEvents()
...
async function revokeAllSessions() {
  ...
  try {
    ...
    await apiClient.revokeAllSessions(refreshToken)
