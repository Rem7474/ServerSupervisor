...
A user-facing error also carries a catalog key: `apperr.Validation("...").I18n(apperr.CodeXxx, params)`. The literal message stays English (it is the dev-facing fallback and what shows up in logs); the user-visible text comes from `ErrorCatalog` in [catalog.go](internal/apperr/catalog.go), which `respondError` renders in the caller's `Accept-Language` and also ships as `i18nKey` so the SPA can translate it into whatever language its UI is set to. Adding a code means three things, all enforced by `internal/apperr/catalog_test.go`: the `CodeXxx` constant, an `ErrorCatalog` entry covering every code in `SupportedLanguages` (with matching `{placeholders}`), and the matching key in `frontend/src/locales/{fr,en}/errors.json`. Not every site is migrated yet — `internal/services/{proxmox,releasetracker,discovery,hostperm}` still return French literals, which reach an English user verbatim; migrate the ones you touch.
