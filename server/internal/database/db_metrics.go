...
func (db *DB) InsertMetrics(m *models.SystemMetrics) (int64, error) {
	...
	err := db.conn.QueryRow(
		`INSERT INTO system_metrics (host_id, timestamp, cpu_usage_percent, cpu_cores, cpu_model,
		 cpu_temperature, load_avg_1, load_avg_5, load_avg_15, memory_total, memory_used, memory_free, memory_percent,
		...
		m.CPUTemperature, m.LoadAvg1, m.LoadAvg5, m.LoadAvg15, m.MemoryTotal, m.MemoryUsed, m.MemoryFree, m.MemoryPercent,
...
func (db *DB) GetLatestMetricsAll() (map[string]*models.SystemMetrics, error) {
	rows, err := db.conn.Query(
		`SELECT DISTINCT ON (host_id) id, host_id, timestamp,
		 ...
		 cpu_temperature, load_avg_1, load_avg_5, load_avg_15,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&m.CPUTemperature, &m.LoadAvg1, &m.LoadAvg5, &m.LoadAvg15,
...
func (db *DB) GetMetricsHistory(hostID string, hours int) ([]models.SystemMetrics, error) {
	rows, err := db.conn.Query(
		`SELECT id, host_id, timestamp, cpu_usage_percent, cpu_cores, cpu_temperature, load_avg_1, load_avg_5, load_avg_15,
...
func (db *DB) GetMetricsAggregatesByType(hostID string, hours int, aggregationType string) ([]models.SystemMetrics, error) {
	rows, err := db.conn.Query(
		`SELECT id, host_id, timestamp, cpu_usage_avg as cpu_usage_percent, 0 as cpu_cores, 0 as cpu_temperature,
