...
ALTER TABLE alert_rules RENAME COLUMN threshold TO threshold_crit;
ALTER TABLE alert_rules RENAME COLUMN threshold_clear TO threshold_clear_crit;
...
COMMENT ON COLUMN alert_rules.threshold_warn IS 'Warning level threshold. Incidents triggered when threshold_warn is exceeded.';
COMMENT ON COLUMN alert_rules.threshold_crit IS 'Critical level threshold. Incidents triggered when threshold_crit is exceeded.';
COMMENT ON COLUMN alert_rules.threshold_clear_warn IS 'Hysteresis threshold for resolving warning incidents.';
COMMENT ON COLUMN alert_rules.threshold_clear_crit IS 'Hysteresis threshold for resolving critical incidents.';
