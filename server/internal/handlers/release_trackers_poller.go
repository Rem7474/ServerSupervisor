...
func (h *ReleaseTrackerHandler) checkOneGit(ctx context.Context, t models.ReleaseTracker) {
	...
	slog.InfoContext(ctx, fmt.Sprintf("Git tracker %s: new release %s (was %s) → dispatching task %s on host %s",
...
func (h *ReleaseTrackerHandler) checkOneDocker(ctx context.Context, t models.ReleaseTracker) {
	...
	slog.InfoContext(ctx, fmt.Sprintf("Docker tracker %s: new digest for %s:%s → dispatching (%s) on host %s",
