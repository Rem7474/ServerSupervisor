...
- **Structured logging** — `internal/logging` wires slog (JSON in prod / text in dev) as the default and bridges the std `log` package through it. `RequestIDMiddleware` assigns a correlation ID per request (X-Request-ID header) that the contextHandler attaches to every log line. Configure via `LOG_LEVEL` / `LOG_FORMAT`.
- **Monolithic files** flagged for split:
  ...
  - frontend: [views/ProxmoxNodeView.vue](frontend/src/views/ProxmoxNodeView.vue) (2384 LoC), [views/TrafficView.vue](frontend/src/views/TrafficView.vue) (1522), [components/alerts/AlertRuleModal.vue](frontend/src/components/alerts/AlertRuleModal.vue) (1321).
