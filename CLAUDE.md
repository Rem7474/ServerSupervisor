...
- **TypeScript migration** — most `.vue` files still use plain JS in `<script setup>`. New components should use `<script setup lang="ts">` (all the split sub-components above are TS). The `utils/` and `composables/` layer is fully `.ts`.
