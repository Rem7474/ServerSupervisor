...
- [internal/poller/](server/internal/poller/) — generic `Every(ctx, interval, immediate, name, tick)` scheduling loop; stops on ctx cancellation.
...
- **Service + repository layer** — DONE. Every HTTP domain lives in `internal/services/<domain>` behind a consumer-defined `Repository` port (structurally satisfied by `*database.DB`); handlers are thin and render via `respondError`. See the `internal/services/` and `internal/apperr/` bullets above. The only handler-held `*DB` is `requireHostAccess` (HTTP authz guard). The `*database.DB` god-object remains by design — the ports already isolate every consumer, so splitting the struct would be cosmetic.
- **Event-driven WebSocket** — DONE. The snapshot endpoints push on write via the `internal/events` bus instead of polling the DB on a 10s timer; a 60s safety rebuild is only a backstop. See the `internal/ws/` bullet.
- **Typed-error envelope** — DONE. All handler error sites go through `respondError`/`apperr`; the wire envelope is uniformly `{"error","code"}`.
...
- **Structured logging** — DONE on **both** modules. Server: `server/internal/logging` wires slog (JSON in prod / text in dev) as the default and bridges the std `log` package through it. `RequestIDMiddleware` assigns a correlation ID per request (X-Request-ID header) that the contextHandler attaches to every log line. Configure via `LOG_LEVEL` / `LOG_FORMAT`. Direct `slog.{Info,Warn,Error}Context(ctx, …)` migration is DONE across `scheduler/`, `alerts/`, `handlers/`, `database/`, `dispatch/`, `notify/`, `ws/`. Agent: `agent/internal/logging` mirrors it (no request-id; text default / JSON via `log_format` config or `SUPERVISOR_LOG_FORMAT`), level via `--verbose` (→debug) or `log_level` / `SUPERVISOR_LOG_LEVEL`; all reporter/sender/dispatcher/collector call sites use structured `slog` (the old threaded `verbose bool` collector param was removed — verbose output is now plain `slog.Debug` gated by level). The `log.SetOutput` bridge in each `logging.go` remains for any third-party or future call sites that use the std package. Background workers no longer swallow errors/panics silently: the uptime/ssl synthetic workers and the alert engine's audit-log writes log failures (the uptime probe `recover()` logs panic+stack like `background/runner.go`).
- **Monolithic files** flagged for split:
  - server: DONE. `alerts/engine.go` split into `engine.go` (orchestration) / `metrics.go` (metric resolvers) / `authfailures.go` (Proxmox syslog auth-failure parsing) / `severity.go` (hysteresis) / `notify.go` (dispatch). `handlers/alert_rules.go` split into `alert_rules.go` (struct + vars + validation) / `_capabilities.go` / `_crud.go` / `_testrun.go` / `_incidents.go`. `handlers/release_trackers.go` split, then its lifecycle/poller/dispatch/notify logic relocated into `internal/services/releasetracker`. The agent ingest hot path (`ReceiveReport`) likewise now lives in `internal/services/agent` (handler is thin). **All flagged server monoliths are resolved.**
...
- **TypeScript migration** — DONE. All `.vue` files use `<script setup lang="ts">`. New code should also be TS. The only residual `any` is `useHostDetail.ts`'s internal `AnyRecord` display refs — intentional and contained (see TypeScript adoption note above); the wire contracts (REST + WS) are fully typed.
