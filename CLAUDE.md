...
- [internal/services/](server/internal/services/) — application/service layer per domain (currently `uptime`): owns business logic behind a `Repository` port (satisfied structurally by `*database.DB`), so it is unit-testable without a database. The proof slice; other domains still call `h.db.*` directly from handlers and can migrate incrementally.
- [internal/apperr/](server/internal/apperr/) — typed domain errors (`NotFound`/`Validation`/`Conflict`/`Forbidden`/`Unauthorized`/`Internal`) with a machine `Code` + HTTP status; `From()` coerces any error. Rendered as `{"error","code"}` by `respondError`.
